...
#panel_Trn  control items:
        ...
        btn_Load_xml_file = wx.Button(panel_Trn, wx.ID_ANY, u"XMLファイルを選択（単数）", size=(150,25))
        self.xml_fn_TextCtrl  = wx.TextCtrl(panel_Trn, ID_TARGET_XSLTFN, style=wx.TE_LEFT)

        btn_Trn_excute = wx.Button(panel_Trn, wx.ID_ANY, u"変換実行", size=(150,25))
        btn_Exit = wx.Button(panel_Trn, wx.ID_EXIT, u"終　了", size=(150,25))#これは明らかにexitボタンです

        self.log_TextCtrl = wx.TextCtrl(panel_Trn, ID_TARGET_LOG, style=wx.TE_LEFT|wx.TE_MULTILINE)
        btn_ClearLog = wx.Button(panel_Trn, wx.ID_ANY, u"メッセージをクリア", size=(150,25))
...
#panel_Trn  layout:
        ...
        sizer.Add(wx.StaticText(panel_Trn, wx.ID_ANY, u"　メッセージ:"))
...
#load files for transform
    ...
    def loadXmlFile(self,event):
        ...
        fileDialogXML = wx.FileDialog(self, u"XMLファイルをロード", defaultFile="*.xml", wildcard=xml_wildCard, style=wx.FD_DEFAULT_STYLE)
